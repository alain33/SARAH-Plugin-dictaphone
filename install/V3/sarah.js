...
var exists = function(cmd){
  if (SARAH.ConfigManager.getConfig().modules[cmd] || cmd == 'time'){
    return true;
  }
  if (SARAH.ConfigManager.getConfig().phantoms[cmd]){
    return true;
  }
  return false;
}
...
var askme = function(tts, grammar, timeout, callback){
  ...
  options.token    = setTimeout(function(){
      options = false;
      if (timeout <= 0){
			if (SARAH.exists('mute'))
				SARAH.call('mute', {command : 'autoMute', values : {Cmd: 'askme', Options: {timeout: true}}});
			callback(false, end);
      } else 
...
var answerme = function(req, res, next){
  ...
  console.log('Answer: ' + req.param('tag'));
...
var end = function(timeout){ 
  ...
  if (timeout == true)
	   remote({ 'context' : 'default' });
  
  if (SARAH.exists('mute'))
	SARAH.call('mute', {command : 'autoMute', values : {Cmd: 'askme', Options: {timeout: timeout}}});
